"""Phase 10: movement analytics on padel court (10×20 m)."""
...
class MovementAnalytics:
    ...
    def compute(self) -> MovementStats:
        ...
        for i in range(1, len(self.positions)):
            ...
            if abs(y1 - net_y) < 2.5:
                net_zone += 1
            elif y1 < 3.5 or y1 > self.court_length - 3.5:
                defensive += 1
        ...
        return MovementStats(
            ...
            net_zone_pct=net_zone / total_frames,
    ...
    def _build_heatmap(self, bins: int = 20) -> list[list[float]]:
        ...
        for _, x, y in self.positions:
            xi = int(np.clip(x / self.court_width * (bins - 1), 0, bins - 1))
