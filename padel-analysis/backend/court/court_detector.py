"""Padel court detection: manual homography + optional line keypoints."""
...
from backend.court.geometry import CANONICAL_COURT, COURT_LENGTH_M, COURT_WIDTH_M, NET_Y_M, SERVICE_DEPTH_M
...
class CourtDetector:
    """Phase 1: homography backbone — detect once, transform everything to court coords."""
    ...
    def set_video_source(self, video_path: str) -> None:
        ...
        if self.use_calibration and self.video_stem:
            ...
            if cal is not None:
                self._cached_h = cal
                logger.info("Loaded manual padel calibration for %s", self.video_stem)
    ...
    def is_in_court(self, cx: float, cy: float) -> bool:
        return 0 <= cx <= self.court_width and 0 <= cy <= self.court_length

    def is_in_service_box(self, cx: float, cy: float, near_end: bool = True) -> bool:
        if not (0 <= cx <= self.court_width):
            return False
        if near_end:
            return 0 <= cy <= SERVICE_DEPTH_M
        return self.court_length - SERVICE_DEPTH_M <= cy <= self.court_length

    def is_at_net(self, cx: float, cy: float, tolerance_m: float = 2.0) -> bool:
        return abs(cy - NET_Y_M) <= tolerance_m and 0 <= cx <= self.court_width

    def court_zone(self, cx: float, cy: float) -> str:
        """Offensive (net), transitional, or defensive (back wall) zone."""
        if not self.is_in_court(cx, cy):
            return "out"
        if cy < self.court_length * 0.35:
            return "defensive_near"
        if cy > self.court_length * 0.65:
            return "defensive_far"
        if abs(cy - NET_Y_M) < 2.5:
            return "net"
        return "transitional"
    ...
    def _detect_white_line_keypoints(self, frame: np.ndarray) -> list[tuple[float, float]]:
        ...
        h, _ = frame.shape[:2]
        pts = [p for p in pts if p[1] > h * 0.2]
        if len(pts) < 4:
            return pts
